...
func spawn(creep_name: String, creep_count: int) -> void:
	if not creep_catalog.has(creep_name):
		push_error("creep with name '" + creep_name + "' not found")
		push_error("found: " + str(creep_catalog.keys()))
	
	for i in range(creep_count):	
		var creep: Creep = _build_creep(creep_catalog[creep_name])
		creep.visible = false
		add_child(creep)
		Event.creep_spawn.emit(creep)
